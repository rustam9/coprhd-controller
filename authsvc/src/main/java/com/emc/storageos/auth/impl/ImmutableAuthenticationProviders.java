...
public class ImmutableAuthenticationProviders {
    ...
    private static boolean checkManagerDNAndSearchBase(LdapTemplate template, final AuthnProviderParamsToValidate param,
        ...
        try {     
            ...
            errorString.append(MessageFormat.format("Connection to LDAP server {0} failed. Please, check the scheme, accessibility of the LDAP server and port. LDAP error: {1}.", 
            ...
            errorString.append(MessageFormat.format("Connection to the LDAP server {0} succeeded but the Manager DN {1} or its password failed to authenticate.  LDAP error: {2}", 
                    param.getUrls().toString(), param.getManagerDN(), stripNonPrintableCharacters(e.getMessage())));
            return false;
        } catch ( NameNotFoundException e ) {
            errorString.append(MessageFormat.format("Connection to the LDAP server {0} succeeded and the Manager DN authenticated successfully but the search base path {1} could not be found in the LDAP tree. LDAP error: {2}", 
                    param.getUrls().toString(), param.getSearchBase(), stripNonPrintableCharacters(e.getMessage())));
            return false;  
        } catch ( PartialResultException e ) {
            errorString.append(MessageFormat.format("Connection to the LDAP server {0} succeeded and the Manager DN authenticated successfully but a portion of the search base path {1} could not be found in the LDAP tree. LDAP error: {2}", 
                    param.getUrls().toString(), param.getSearchBase(), stripNonPrintableCharacters(e.getMessage())));
            return false;
        } catch (Exception e) {
            errorString.append(MessageFormat.format("Validation of the Manager DN {0} and search base {1} against the LDAP server {2} failed because of LDAP error: {3}", 
    ...
    private static boolean checkGroupAttribute(LdapTemplate template, final RootDSE rootDSE, final AuthnProviderParamsToValidate param,
        ...
        try {
            ...
            errorString.append(MessageFormat.format("Connection to LDAP server {0} failed during search for group attribute {1}. LDAP error: {2}", 
                    param.getUrls().toString(), param.getGroupAttr(), stripNonPrintableCharacters(e.getMessage())));
            return false;
        } catch (Exception e) {
            errorString.append(MessageFormat.format("Validation of group attribute {0} against server {1} failed because of LDAP error: {2}", 
    ...
    private static boolean checkDirectoryType(LdapTemplate template, RootDSE rootDSE, final AuthnProviderParamsToValidate param,
        ...
        if (rootDomainNamingContext == null
            ...
            if (! param.getMode().equals("ad")) {
                ...
                String errorMsg = MessageFormat.format("Directory server type Active Directory doesn't match with specified mode {1} at server {0}.", 
                        param.getUrls().toString(), param.getMode());
                _log.warn(errorMsg);
            }
            ...
            try {
                ...
                String infoMsg = MessageFormat.format("Active Directory server information {0} - server type: {1}, objectVersion: {2}, Microsoft Windows Server version: {3}", 
                        param.getUrls().toString(), serverType, objectVersion, windowsServer);
                _log.info(infoMsg); 
                return true;
            } catch ( CommunicationException e ) {
                String errorMsg = MessageFormat.format("Connection to Active Directory server {0} failed during query of schema DN ({1})'s objectVersion attribute. LDAP error: {2}", 
	...
	private static List<List<String>> searchInLDAPSchema(LdapTemplate template, String[] returnAttributes,
		...
		try {
            ...
            errorString.append(MessageFormat.format("Connection to LDAP server {0} failed during search for attribute {1}. LDAP error: {2}", 
            		ldapServerUrls.toString(), returnAttributes.toString(), stripNonPrintableCharacters(e.getMessage())));
            return null;
        } catch (Exception e) {
            errorString.append(MessageFormat.format("Exception during attribute {0} search against server {1} failed because of LDAP error: {2}", 
