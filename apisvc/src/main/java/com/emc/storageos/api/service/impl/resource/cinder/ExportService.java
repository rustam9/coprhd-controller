...
public class ExportService extends VolumeService {
	...
	public Object actionOnVolume(@PathParam("tenant_id") String openstack_tenant_id,
		...
		_log.info(String.format("bReserve:  %b , bUnReserve:  %b, bTerminate:%b, bBeginDetach:%b , bDetach:%b , " +
				"bAttach:%b , bInitCon:%b , bExtend:%b", bReserve,bUnReserve,bTerminate,bBeginDetach, bDetach, bAttach,  bInitCon,bExtend) );		
		...
		if( (bInitCon) && (action.attach.connector!=null) && (action.attach.connector.ip!=null) && (action.attach.connector.ip.length() > 0)){				
			...
			if(bIsSuccess){
				...
				//If the protocol is FC
					...
					List<Initiator> lstInitiators = getListOfInitiators(action.attach.connector, openstack_tenant_id, chosenProtocol);
	...
	private boolean  processDetachRequest(Volume vol, 
		...
		List<Initiator> detachInitiators = getListOfInitiators(detach.connector, openstack_tenant_id, protocol);
		...
		exportController.exportGroupUpdate(exportGroup.getId(), volumeMap, new ArrayList<URI>(),
				new ArrayList<URI>(), currentURIs, task);
	...
	private boolean processAttachRequest(Volume vol, VolumeActionRequest.AttachVolume attach, 
		...
		List<Initiator> newInitiators = getListOfInitiators(attach.connector, openstack_tenant_id, protocol);
		...
		if (exportGroup != null) {
			...
			// Now update export group
			exportController.exportGroupUpdate(exportGroup.getId(), volumeMap, new ArrayList<URI>(),
					new ArrayList<URI>(), initiatorURIs, task);		
		}
		else {
	...
	private List<Initiator> getListOfInitiators(Connector connector, String tenant_id, String protocol) {
